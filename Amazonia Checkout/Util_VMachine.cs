...
namespace ValidationMachine
{
    ...
    {
        public Dictionary<VM_Type, object[]> Validators = new Dictionary<VM_Type, object[]>();

        public Dictionary<(VM_Status, bool), System.Action> FeedbackActions = new Dictionary<(VM_Status, bool), System.Action>();

        public List<VM_Status> Statuses = new List<VM_Status>();
        ...
        /// <summary>
        /// Tests if target.Text contains at least 1 capital letter
        /// </summary>
        private void CapitalTest()
        {
            if (!Target.Text.Any(char.IsUpper))
            {
                Statuses.Add(VM_Status.No_Capital);
            }
        }
        ...
        /// <summary>
        /// Tests if target.Text equals the Text of a specified control
        /// </summary>
        /// <param name="control">Control to compare</param>
        private void EqualControlTest(Control control)
        {
            if(Target.Text != control.Text)
            {
                Statuses.Add(VM_Status.Not_Equal_Control);
            }
        }
        ...
        public List<VM_Status> Validate()
        {
            ...
            foreach(var validator in Validators)
            {
                switch (validator.Key)
                {
                    ...
                    case VM_Type.Equal_Control:
                        if(validator.Value != null && validator.Value[0] is Control control)
                        {
                            EqualControlTest(control);
                        }
                        else
                        {
                            Statuses.Add(VM_Status.Bad_Equal_Control_Args);
                        }
                        break;
